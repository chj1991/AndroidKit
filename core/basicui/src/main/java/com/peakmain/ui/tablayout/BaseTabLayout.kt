...
abstract class BaseTabLayout<T> : HorizontalScrollView {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        // 无 XML attrs 时也要有默认色，否则 ColorTrack 进度为透明
        mOriginColor = Color.BLUE
        mChangeColor = Color.RED
        mIsShowUnderLine = false
        if (attrs != null) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.BaseTabLayout)
            mOriginColor = ta.getColor(R.styleable.BaseTabLayout_originColor, mOriginColor)
            mChangeColor = ta.getColor(R.styleable.BaseTabLayout_changeColor, mChangeColor)
