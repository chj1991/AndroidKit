...
class CustomPopupWindow private constructor(private val mContext: Context) :
    ...
    private fun build(): PopupWindow {
        ...
        // 2017.3.17 add
        // 获取当前Activity的window（兼容 ContextThemeWrapper）
        if (mIsBackgroundDark) {
            val activity = findActivity(mContentView!!.context)
            if (activity != null) {
                //如果设置的值在0 - 1的范围内，则用设置的值，否则用默认值
                val alpha =
                    if (mBackgroundDrakValue > 0 && mBackgroundDrakValue < 1) {
                        mBackgroundDrakValue
                    } else {
                        DEFAULT_ALPHA
                    }
